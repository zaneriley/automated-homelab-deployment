...
The `scheduler.yml` + `backup.yml` pair (cron-on-each-host pattern) is being retired in favour of master-driven orchestration: the cron lives on the master node and SSHes out to fleet hosts to run backups. Two drivers: (1) the centralised model is consistent with ADR-0001 ("one harness for the whole fleet") — the scheduler runs from one place that knows which fleet hosts are reachable; (2) the cron-on-each-host pattern is awkward to manage centrally. See `scheduler.yml`'s `ARCHITECTURE PIVOT` block for the full rationale and the AGENTS.md `Later` backlog item that tracks the rethink.
